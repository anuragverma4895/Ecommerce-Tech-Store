...
for (let i = 0; i < lines.length; i++) {
  // Detect category
  const catMatch = lines[i].match(/category:\s*["'](\w+)["']/);
  if (catMatch) {
    currentCategory = catMatch[1];
    if (!categoryCounters[currentCategory]) categoryCounters[currentCategory] = 0;
  }
  
  // Detect image line and replace
  const imgMatch = lines[i].match(/^(\s*image:\s*)"[^"]+"(,?\s*)$/);
  if (imgMatch && currentCategory && imagesByCategory[currentCategory]) {
    const imgs = imagesByCategory[currentCategory];
    const idx = categoryCounters[currentCategory] % imgs.length;
    lines[i] = `${imgMatch[1]}"${imgs[idx]}"${imgMatch[2]}`;
...
console.log(`Replaced ${replacedCount} image URLs`);
